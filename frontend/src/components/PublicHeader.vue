<template>
  <div class="publicheader">
    ...
    <div id="bookmark-switch">
      <v-switch
        v-model="bookMarkSwitch"
        :label="bookMarkStatus"
        color="orange"
      ></v-switch>
    </div>

    <div id="functional-button-group">
      <v-menu offset-y>
        ...
        <v-list>
          <v-list-item v-for="(item, index) in disturbs" :key="index">
      ...
      <v-dialog v-model="dialog" persistent max-width="600px">
        <template v-slot:activator="{ on, attrs }">
          <v-btn v-bind="attrs" v-on="on" id="sign-in-btn">
...
export default class CheckFile extends Vue {
  ...
  //computed
  get bookMarkStatus(): string {
    if (this.bookMarkSwitch) {
      return "BookMark";
    }
    return "News";
  }
...
#sign-in-btn{
  background: linear-gradient(270deg,#2f8fef, #2fa7b7);
  color: #fff;
}
</style>
